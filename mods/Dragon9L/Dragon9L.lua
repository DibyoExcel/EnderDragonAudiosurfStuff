    		...
    		for k=1,#traffic do
    			if (traffic[k].chainstart <= powerupImpactNode) and (traffic[k].chainend >= powerupImpactNode) and (traffic[k].type < 100) then
    				while traffic[k].lane == powerupLane do
    					traffic[k].lane = math.random(-4, 4)
    				end
    			end
    		end
    	end
...
function Collide(strafe, tracklocation)
	...
	elseif (strafe>half_lanespace+(half_lanespace*2) and strafe<half_lanespace+(half_lanespace*4)) then 
		playerLane = 2
	elseif (strafe>half_lanespace+(half_lanespace*4) and strafe<half_lanespace+(half_lanespace*6)) then 
		playerLane = 3
	elseif strafe>half_lanespace+(half_lanespace*6) then
		playerLane = 4
	elseif (strafe<-half_lanespace and strafe>-(half_lanespace+(half_lanespace*2))) then 
		playerLane=-1
	elseif (strafe<-(half_lanespace+(half_lanespace*2)) and strafe>-(half_lanespace+(half_lanespace*4))) then 
					...
					if not blocks[i].hidden then
						if (blocks[i].lane == playerLane) then
							...
							if not isPowerup then
								SetPuzzle{newblocks={{type=blocks[i].type, collision_strafe=blockOffset[1], puzzle_col=blocks[i].lane+4, add_top=true}}}
							else
