...
GameplaySettings{
        ...
        uphilltiltscaler = 4,--set to 1 for normal track. higher for steeper
        downhilltiltscaler = 4,--set to 1 for normal track. higher for steeper
...
function InsertLoopyLoop(theTrack, apexNode, circumference)
    ...
    local tiltDeltaOverEntireLoop = -720 + (endOriginalTilt - startTilt)
...
function InsertCorkscrew(theTrack, startNode, endNode)
	...
	if endNode < #theTrack then
		...
		local rollIncrement = (1080-(2160*math.random(0, 1))) / (endNode-startNode)
